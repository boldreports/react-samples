...
const fs = require('fs');
let customNames = JSON.parse(fs.readFileSync('src/samples.json', 'utf8')).customNames;
let checkedFiles = {
    folders: [],
    files: [],
    upperCase: []
...
gulp.task('file-validation', function (done) {
    ...
    let fileNames = glob.sync('src/**/*.*', {
        ignore: ['src/App.js', 'src/rdlcData.js', 'src/reportWebVitals.js', 'src/**/*.png', 'src/**/*.svg', 'src/**/*.json', 'src/**/*.eot', 'src/**/*.ttf',
            'src/**/*.woff', 'src/**/*.gif', 'src/**/*.jpg', 'src/**/*.JPG', 'src/**/*.PNG', 'src/styles/ej2-buttons.min.css'
        ]
    });
...
function checkFileNames(filePath, isFiles) {
    ...
    for (let j = 0; j < splitted.length; j++) {
        if (!dictionary.check(splitted[j]) && customNames.indexOf(splitted[j]) === -1) {
            splitted.join(' ').replace(' ', '') == 'sbicons' ? '' : isValid = false;
        }
